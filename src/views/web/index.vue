        ...
        <form v-show="mode === 'register'" class="auth-form is-active" novalidate @submit.prevent="submitRegister">
          ...
          <div class="field-grid">
            ...
            <label class="field field--code" :class="fieldClass('code')">
              ...
              />
              <button class="secondary-action" type="button" :disabled="codeCountdown > 0" @click="startCodeCountdown">
...
function validateField(field) {
  ...
  if ((typeof value === 'string' && !value.trim()) || value === false) {
    ...
    message = '两次输入的密码不一致。'
  } else if (field === 'code' && !/^\d{6}$/.test(value)) {
...
function simulateSubmit(successMessage) {
  clearStatus()
  loading.value = true
  window.setTimeout(() => {
    loading.value = false
...
function submitLogin() {
  if (!validateFields(['loginAccount', 'loginPassword'])) {
    showStatus('error', '请检查表单中标记的内容后继续。')
    return
  }

  const roleName = role.value === 'teacher' ? '教师端' : '学生端'
  simulateSubmit(`${roleName}登录成功，正在进入工作台。`)
}
...
function submitRegister() {
  ...
  simulateSubmit(`${roleName}账号创建成功，请继续完善资料。`)
}
